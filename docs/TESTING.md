...
1. Build **FoldCounter Duo** with stable Xcode 26.6 for distribution, then run it on the iOS 27.1 Duo simulator with Xcode 27.1. Confirm that the public `UIHingeInteraction` runtime bridge delivers optional angles, and verify angle units/endpoints and API availability against Apple's current declarations. Update only the adapter when normalization is needed.
